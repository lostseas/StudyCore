...
{
    public abstract class BaseRepository<TEntity, T> : IRepository<TEntity, T> where TEntity : BaseModel<T> where T : struct
    {
        ...
        public virtual async Task<int> UpdateListAsync(List<TEntity> list)
        {
            try
            {
                if (list.Any())
                {
                    ...
                    {
                        bool flag = propertyInfo.GetValue(entity) != null || propertyInfo.GetValue(entity).ToString().Equals(DateTime.MinValue.ToString());
                        if (flag)
                        {
                    ...
                    foreach (var current in list)
                    {
                        ...
                        foreach (var filed in modifyFiled)
                        {
                            context.Entry(current).Property(filed).IsModified = true;
                        }
                    }
        ...
        private static List<string> GetModifyFiled<TModel>(TModel entity) where TModel : class, new()
        {
